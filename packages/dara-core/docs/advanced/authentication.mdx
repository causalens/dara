...
Each `AuthComponent` specifies `js_source`, the module that default-exports the screen, and `py_module`, the Python package used to determine its dependency version. Dara uses `js_source` to look up the screen before authentication. Multiple routes can share the same source, and the exported function can have any name.
...
MyLoginComponent = AuthComponent(
    js_source='my-custom-auth-package/login',
    py_module='my_custom_auth'
)

MyLogoutComponent = AuthComponent(
    js_source='my-custom-auth-package/logout',
    py_module='my_custom_auth'
)

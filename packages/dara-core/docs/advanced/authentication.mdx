...
`FileAuthSessionBackend()` also accepts `DARA_AUTH_SESSION_FILE_PATH` as its default directory override when no `path` is passed. Explicit and environment-provided directories must already exist. When no path is configured, Dara creates an app-scoped directory under the platform temporary directory.

The file backend stores one complete session snapshot per file. Filenames are hashes of the opaque browser session token, but file contents include raw auth token material and should be treated as sensitive. The file backend is local, single-host storage. It is useful for local reload loops, demos, and single-host proof-of-concept deployments. For production deployments, use the in-memory backend if logging users out on restart is acceptable; if sessions must survive restarts, prefer a durable database or shared cache backend when available.

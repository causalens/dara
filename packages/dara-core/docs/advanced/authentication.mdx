...
The `extract_user_data` method receives an `IdTokenClaims` object containing all claims from the ID token, an optional `userinfo` dictionary (when `SSO_USE_USERINFO=true` and an access token is available), and should return a `UserData` object:
...
class CustomOIDCAuthConfig(OIDCAuthConfig):
    def extract_user_data(
        ...
        # When SSO_USE_USERINFO is enabled, userinfo may contain additional claims
...
:::note
The `userinfo` parameter is only populated during initial login (`sso_callback`) and token refresh, when an access token is available to call the userinfo endpoint. When verifying raw IDP tokens directly (e.g., for programmatic API access), `userinfo` will be `None`.
:::

...
Both configurations present a login form where users enter their credentials. Upon successful authentication, Dara sets a signed session cookie.
...
7. Dara sets a signed session cookie

### Handling Custom Claims

Identity providers may return claims in non-standard formats or use custom claim names. To handle these cases, you can subclass `OIDCAuthConfig` and override the `extract_user_data` method to customize how claims are mapped to user data.

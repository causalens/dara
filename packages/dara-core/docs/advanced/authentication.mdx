...
For backwards compatibility, Dara still issues a session token in auth responses and the frontend continues to send a bearer header when available. This allows existing custom extensions that explicitly inspect `Authorization` headers to keep working during migration.
...
Both configurations present a login form where users enter their credentials. Upon successful authentication, Dara sets a signed session cookie and also returns a JWT session token for compatibility.
...
7. Dara sets a signed session cookie and returns a session token for compatibility

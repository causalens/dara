...
```toml
[project.entry-points."dara_assets"]
...
Make sure to include the files inside your wheel in your `pyproject.toml` file. With Hatchling, the package directory is included by default - but if the generated assets are covered by a `.gitignore`, list them in the wheel target's `artifacts` so they are still shipped:

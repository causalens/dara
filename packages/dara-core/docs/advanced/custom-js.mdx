...
Make sure to include the files inside your wheel in your `pyproject.toml` file - e.g. Hatchling includes everything inside the package directory by default, while setuptools requires a `package-data` entry:

...
In most cases you should prefer the `Link` component over the `navigate` action as it is more accessible and performant. The action form is mostly required in very dynamic scenarios where you need to navigate only under some circumstances or the target URL needs to be computed in the action itself.

See the [Routing](./routing) section for more details.
...
Use `ctx.download_file()` for static file paths as well; it creates the download URL and opens it through the standard navigation action.

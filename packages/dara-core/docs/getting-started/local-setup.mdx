...
- Python v3.10+
- NodeJS v20.19+ (Don't worry - you don't need to understand any JS, this is used to build the application in production mode)
...
:::warning
The package name cannot have hyphens, and therefore you must substitute `-` for `_`. For example, the `first-dashboard` project will have a `first_dashboard` package. Set `[tool.setuptools] packages` to the same value.
:::

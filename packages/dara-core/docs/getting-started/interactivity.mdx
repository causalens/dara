...
In cases where you want to simply display different components based on a predefined set of values that your dynamic variable can take, using `If` would be cumbersome as you would need to nest multiple instances of it. It would also not be very efficient as the same variable would be used at each layer:


```python
from enum import StrEnum
from dara.core import Variable
from dara.components import If, Text
...
If(
    condition=var_status == Status.OK,
    true_children=Text('OK'),
    false_children=If(
        condition=var_status == Status.ERROR,
        true_children=Text('ERROR'),
        false_children=If(
            condition=var_status == Status.WARNING,
            true_children=Text('WARNING'),
            false_children=Text('Unknown status')
        )
    )
)
```

This can be simplified by using the `Match` component to "map" values to different UI elements:
...
In very simple cases like the above, you can also use `SwitchVariable` instead of `Match` to achieve the same result, i.e. `Text(SwitchVariable.match(...))`, since we are just changing the value of the `Text`. `Match` is useful in all more complex cases though where you might want to change which component is displayed depending on the case matched.
...
`LoopVariable` also supports the `is_first`, `is_last` and `index` "meta" properties which can be useful to change part of the display depending on the item's position in the list.

```python
from dara.core import Variable, For, If
from dara.components import Text, Stack

my_list = Variable([1, 2, 3])

# Renders a list of Text components joined by '>'
For(
    items=my_list,
    renderer=Stack(
        If(
            my_list.list_item.is_first != True,
            Text('>')
        ),
        Text(text=my_list.list_item),
...
Do not use `@py_component` inside a `For` renderer. This defeats the purpose of the optimized loop - each item would require a separate backend call, making the component slower than just using a `py_component` directly.

If you need to transform your data before rendering, precompute the fields in the `items` variable using a `DerivedVariable`:

```python
# AVOID: py_component inside For
@py_component
def format_item(item):
    return Text(f"Formatted: {item['name'].upper()}")

For(items=my_list, renderer=format_item(my_list.list_item))  # This will error

# GOOD: precompute in DerivedVariable, runs once for the whole list
def preprocess_items(items):
    return [{'formatted_name': item['name'].upper(), **item} for item in items]

processed_items = DerivedVariable(preprocess_items, variables=[my_list])

For(
    items=processed_items,
    renderer=Text(processed_items.list_item['formatted_name'])
)
```
...
For real-time data that updates continuously—like live events, notifications, or monitoring data—you can use a `StreamVariable`. It connects to the server using Server-Sent Events (SSE) and receives updates as they happen, without polling.

```python
from dara.core import StreamVariable, StreamEvent, Variable

category = Variable('alerts')

async def events_stream(category: str):
    # Set initial state atomically (handles reconnection correctly)
    yield StreamEvent.replace(*await api.get_events(category))
    # Stream new events as they arrive
    async for event in api.stream_events(category):
        yield StreamEvent.add(event)

events = StreamVariable(events_stream, variables=[category], key_accessor='id')
```

This is useful when you need to display data that changes frequently and want updates to appear immediately without the user refreshing. Learn more in [Advanced: Stream Variables](../advanced/stream-variable.md).
...
The `py_component` function should be thought of as an **escape hatch** from the optimized declarative rendering that Dara provides. In the majority of cases you should not need to use this API. Check out [You Might Not Need `py_component`](../best-practices/you-might-not-need-py-components.md) to learn more.

:::

For the rare cases where you might need to use `py_component`, let's understand how it works.
...
- A `polling_interval` causing the component to poll the backend and refresh itself every `n` seconds. This can be
  either a fixed integer or a `ClientVariable` (for example `SwitchVariable`) for dynamic polling behavior.

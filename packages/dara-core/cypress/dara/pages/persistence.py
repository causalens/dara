...
def persistence():
    ...
    simple_scenario = Stack(
        Text('PERSISTED_VALUE:'),
        Input(value=persisted_value),
        Text('STATIC_VALUE:'),
        Input(value=static_value),
    )
    ...
    nested_scenario = Stack(
        Text('PERSISTED_VALUE:'),
        Input(value=nested_persisted_value.get('nested').get('value')),
    )

    return Stack(
        Card(simple_scenario, title='Simple scenario'),
        Card(nested_scenario, title='Nested scenario'),
    )

...
-   **Backported** Fix an issue where `get_settings` would crash when attempting to generate a missing `.env` file. It now
instead warns and falls back to using an in-memory set of default settings.

## 1.6.6

-   **Backported** Added the ability to pass an asynchronous function to `ConfigurationBuilder.on_startup(...)`.

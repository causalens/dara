...
def backend_store_message(payload: dict[str, Any], patch: bool = False) -> DaraServerMessage:
    """Create the expected self-discriminating backend-store notification."""
    typename: ServerMessageTypename = 'BackendStorePatchMessage' if patch else 'BackendStoreMessage'
    return DaraServerMessage.create(typename, payload)
        ...
        [
            call(
                ...
                backend_store_message(
                    {'store_uid': user_backend_store.uid, 'value': 'test_value', 'sequence_number': 1}
                ),
                ignore_channel=None,
            )
        ]
    ...
    mock_ws_mgr.send_message_to_user.assert_has_calls(
        [
            call(
                USER_2.identity_id,
                backend_store_message(
                    {'store_uid': user_backend_store.uid, 'value': 'test_value_2', 'sequence_number': 1}
                ),
                ignore_channel=None,
            )
        ],
...
async def test_init_with_default(backend_store):
    ...
    assert await backend_store.read() == 'default_value', (
...
async def test_init_with_default_user(user_backend_store):
    ...
    # Verify the store now contains the default value
    assert await user_backend_store.read() == 'default_value', (
        'The store should be initialized with the default value if it was empty.'
    )
...
async def test_notify_on_write(backend_store, mock_ws_mgr):
    ...
    mock_ws_mgr.broadcast.assert_called_once_with(
        backend_store_message({'store_uid': backend_store.uid, 'value': 'test_value', 'sequence_number': 1}),
        ignore_channel=None,
    )
...
async def test_notify_on_delete(backend_store, mock_ws_mgr):
    ...
    mock_ws_mgr.broadcast.assert_called_once_with(
        backend_store_message({'store_uid': backend_store.uid, 'value': None, 'sequence_number': 1}),
        ignore_channel=None,
    )
...
async def test_backend_subscribe_user(custom_backend, mock_ws_mgr: AsyncMock):
    ...
    mock_ws_mgr.send_message_to_user.assert_called_with(
        USER_1.identity_id,
        backend_store_message({'store_uid': store.uid, 'value': 'external_value_1', 'sequence_number': 0}),
        ignore_channel=None,
    )
    assert mock_ws_mgr.send_message_to_user.call_count == 1
    ...
    mock_ws_mgr.send_message_to_user.assert_called_with(
        USER_2.identity_id,
        backend_store_message({'store_uid': store.uid, 'value': 'external_value_2', 'sequence_number': 0}),
        ignore_channel=None,
    )
    assert mock_ws_mgr.send_message_to_user.call_count == 1
...
async def test_backend_subscribe_multiple_stores(custom_backend, mock_ws_mgr):
    ...
    mock_ws_mgr.broadcast.assert_any_call(
        backend_store_message({'store_uid': store1.uid, 'value': 'external_value', 'sequence_number': 0}),
        ignore_channel=None,
    )
    mock_ws_mgr.broadcast.assert_any_call(
        backend_store_message({'store_uid': store2.uid, 'value': 'external_value', 'sequence_number': 0}),
        ignore_channel=None,
    )
...
async def test_write_partial_nested_objects(backend_store):
    ...
    initial_data = {
        'user': {'profile': {'name': 'John', 'age': 30}, 'settings': {'theme': 'light'}},
        'items': ['apple', 'banana'],
    }
    ...
    expected = {
        'user': {'profile': {'name': 'John', 'age': 31, 'city': 'New York'}, 'settings': {'theme': 'dark'}},
        'items': ['banana', 'cherry'],
    }
...
async def test_write_partial_empty_store(backend_store):
    ...
    patches = [{'op': 'add', 'path': '/name', 'value': 'John'}, {'op': 'add', 'path': '/age', 'value': 30}]
...
async def test_write_partial_user_scope(user_backend_store, mock_ws_mgr):
    ...
    patches = [{'op': 'replace', 'path': '/age', 'value': 31}, {'op': 'add', 'path': '/city', 'value': 'New York'}]
...
async def test_write_partial_with_full_object(backend_store, mock_ws_mgr):
    ...
    call_args = jsonable_encoder(mock_ws_mgr.broadcast.call_args[0][0])['message']
...
async def test_write_partial_copy_and_move_operations(backend_store):
    ...
    """
    initial_data = {'source': {'value': 'test_value'}, 'items': ['a', 'b', 'c'], 'target': {}}
    await backend_store.write(initial_data, notify=False)
...
async def test_sequence_number_in_patch_message(backend_store, mock_ws_mgr):
    ...
    if backend_store.scope == 'global':
        ...
        message = jsonable_encoder(call_args[0])['message']
    ...
    if backend_store.scope == 'global':
        ...
        second_message = jsonable_encoder(second_call_args[0])['message']
...
async def test_sequence_number_user_subscription_notifications(mock_ws_mgr):
    ...
    message = jsonable_encoder(call_args[1])['message']
    assert 'value' in message

...
async def test_nested_switch_variable_resolution():
    """
    Test that a SwitchVariable with nested property can resolve nested values on the backend.
    """
    from contextvars import ContextVar
    ...
    def calc(switch_result):
        # switch_result should be the nested value
        return switch_result + 100
    ...
    def page():
        ...
        # Create a switch variable that returns a dict
        switch_var = SwitchVariable.when(
            condition=var1,
            true_value={'nested': {'value': 50}},
            false_value={'nested': {'value': 25}},
            uid='switch_nested_uid',
        )

        # Note: SwitchVariable doesn't have .get(), but the frontend can still send
        # nested property in the resolved variable payload
        derived = DerivedVariable(func, variables=[switch_var])
        dv.set(derived)
        return MockComponent(text=derived)
    ...
    async with AsyncClient(app) as client:
        # Test with condition=True, nested path should extract 50
        # outer_func receives 50 and returns 50 + 100 = 150
        response = await _get_derived_variable(
            client,
            dv.get(),
            {
                'values': [
                    {
                        ...
                        'uid': 'switch_nested_uid',
                        'value': True,
                        'value_map': {True: {'nested': {'value': 50}}, False: {'nested': {'value': 25}}},
                        'default': {'nested': {'value': 0}},
                        'nested': ['nested', 'value'],
                    },
                ],
                'ws_channel': 'test_channel',

...
class PendingComputation:
    """
    An internal class that represents a pending computation that can resolve to either a value or a PendingTask.
    This allows early placeholder setting before we know if the computation will run as a task or direct execution.
    """
    ...
    async def wait(self):
        """
        Wait for the computation to resolve to either a value or PendingTask, then return the appropriate result
        """
        # Waiting in chunks as otherwise Jupyter blocks the event loop
        while not self.event.is_set():
            await anyio.sleep(0.01)
        ...
        if isinstance(self._resolved_to, PendingTask):
            return await self._resolved_to.run()
        elif isinstance(self._resolved_to, PendingValue):
            return await self._resolved_to.wait()
        else:
            # Direct value
            return self._resolved_to

    def resolve_to_value(self, value: Any):
        """
        Resolve the computation to a direct value
    ...
    def resolve_to_pending_value(self) -> PendingValue:
        """
        Convert this computation to a PendingValue and return it for further resolution

        :return: PendingValue that can be resolved later
        """
        if self.event.is_set():
            raise RuntimeError('Cannot convert already resolved PendingComputation')

        pending_value = PendingValue()
        self._resolved_to = pending_value
        self.event.set()
        return pending_value

    def resolve_to_pending_task(self, pending_task: PendingTask):
        """
        Convert this computation to a PendingTask

        :param pending_task: the PendingTask to resolve to
        """
        if self.event.is_set():
            raise RuntimeError('Cannot convert already resolved PendingComputation')

        self._resolved_to = pending_task
        self.event.set()

    def error(self, exc: Exception):
        """
        Resolve the computation with an error
    ...
    def is_resolved(self) -> bool:
        """
        Check if the computation has been resolved
        """

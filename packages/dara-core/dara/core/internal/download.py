...
DownloadRegistryEntry = CachedRegistryEntry(uid='_dara_download', cache = Cache.Policy.TTL(ttl=60*10)) # expire the codes after 10 minutes
...
async def generate_download_code(file_path: str, cleanup_file: bool) -> str:
    ...
    await store.set(DownloadRegistryEntry, key=uid, value=DownloadDataEntry(
        uid=uid,
        file_path=file_path,

...
def create_component_definition(component: type[ComponentInstance]):
    ...
    if component.js_source is None:
        raise ValueError(
            f'{component.__module__}.{component.__qualname__} must define js_source; run dara lock for legacy declarations'
        )
...
def create_action_definition(action: type[ActionImpl]):
    ...
    if action.js_source is None:
        raise ValueError(
            f'{action.__module__}.{action.__qualname__} must define js_source; run dara lock for legacy declarations'
        )
    return ActionDef(

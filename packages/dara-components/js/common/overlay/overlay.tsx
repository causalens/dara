...
const OverlayWrapper = injectCss(styled.div<RenderProp>`
    position: absolute;
    z-index: 2000;
    inset: ${(props) => (props.position?.includes('top') ? '0' : null)} ${(props) => (props.position?.includes('right') ? '0' : null)} ${(props) => (props.position?.includes('bottom') ? '0' : null)} ${(props) => (props.position?.includes('left') ? '0' : null)};

...
config.router.add_page(path='causal-graph', content=causal_graph_viewer_content)
```
...
config = ConfigurationBuilder()
config.router.add_page(path='skeleton', content=CausalGraphViewer(causal_graph=skeleton))
```

...
wheat_data = pd.DataFrame(
    {
        ...
        'wheat_class': ['araraticum', 'araraticum', 'araraticum', 'polonicum', 'polonicum', 'carthlicum', 'carthlicum'],
    }
)
...
def interactive_wheat_scatter_plot():
    p = figure(title='Area vs Perimeter - click the dots', tools='reset', sizing_mode='stretch_both')
...
return Bokeh(p, events=[('CLICK', [selected_variety_var.sync()])])
```
...
config = ConfigurationBuilder()
config.router.add_page(path='lasso', content=lasso_page())
```

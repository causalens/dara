...
- `dara lock` and `dara dev` convert understood legacy project settings after loading the app. The migration skill guides source and command changes before preparation.
...
Use the migration skill to update existing application declarations and scripts. Then run `dara lock` to convert understood legacy project settings and prepare the frontend, and review the diff. `dara dev` performs the same settings conversion during preparation. See [Migration](#migration) for the conversion contract.
...
The proxy preserves encoded paths and queries, streams bodies unchanged, and removes connection-local headers in both directions. HTTP connections have a 10-second connection deadline, 60-second upload/write-idle deadline and 120-second read-idle deadline; browser disconnects cancel response forwarding as soon as the upload has finished. An incomplete upload returns 408 after its idle deadline. An upstream failure before response headers returns 503; a failure after headers aborts the existing response. WebSocket forwarding preserves subprotocols and valid close statuses, translates local abnormal-disconnect indicators into legal close frames, and closes each connection from one place after cancelling both forwarding tasks. Upgrade and close handshakes have bounded deadlines, and cleanup remains bounded when the ASGI request is cancelled.
...
Dara 2.0 removes the legacy pipeline in the same release that introduces this one. The [dara-2-migration skill](https://github.com/causalens/dara/tree/master/skills/dara-2-migration) guides agents through source, configuration, package and deployment changes. The [public migration guide](../../packages/dara-core/docs/dara-2-migration.md) covers the user workflow.

### Project migration

- Source changes belong to the agent or developer: set `[tool.dara] config`, replace legacy declarations with explicit default-export `js_source` modules, preserve registrations and runtime identities, move local sources under `js/`, carry setup and styles into `js/index.tsx`, and update commands according to their purpose.
- After the application loads with current declarations, `dara lock` and `dara dev` convert understood `dara.config.json` settings under the normal preparation lock. Automatic conversion accepts the standard `js/` entry directory, a recognized package manager and string dependency requirements. Unknown settings require manual resolution before the legacy file is removed.
- Copy `extra_dependencies` into ordinary `package.json` dependencies, preserving existing declarations in every dependency section and reporting conflicts. Copy successfully before removing the old configuration. Check file snapshots before writes, preserve permissions and unrelated fields, and allow retries after interruption. Existing npm/Yarn lockfiles remain for review against the new pnpm resolution.
- Automatic conversion has no source analyzer, AST rewrite, configuration inference, command rewriting or directory relocation. Repeated conversion is a quiet no-op. Normal preparation creates missing frontend project files and reconciles the Dara catalog.
- Frozen development reports required conversion without writes. Backend-only debugging skips frontend preparation. `check`, `build` and `start` never apply conversion. There is no separate migration or preview command.
- The skill requires checks, builds and affected user flows in both development and production. Package authors additionally verify that packed npm exports resolve compiled output and Python wheels retain genuine static assets. Generated UMD bundles and `_assets/auto_js/` leave the wheels.
- Removed APIs point to the migration skill. `dara setup-custom-js` reports that normal preparation provides JavaScript support for every app.
...
- Migration fixtures cover dependency conversion, unsupported settings, existing dependency sections, conflicts, frozen mode and repeated execution. Conversion preserves user sources and scripts, checks for concurrent edits, and can resume after interrupted writes. A packed application first rejects legacy declarations without changing files, then applies the source changes described by the skill and passes configuration conversion, checking, building and browser interaction.
...
The existing router preloads route templates, action definitions, derived variables and Python components through `usePreloadRoute`, triggered by link hover, focus or touch. Navigation consumes the cached route data. This remains supported with the eager 2.0 entry; the deferred work is to preload JavaScript implementations through the same flow.

Start with an end-to-end experiment using one heavy component. Give each lazy component registration a stable loader shared by its `React.lazy` wrapper and an explicit preload operation. Both callers reuse the same import promise. Keep this module cache separate from the router's short-lived, single-use data cache, and define how speculative preload failures are handled and how rendering can retry failed loads.

When the route template arrives in `fetchRouteData`, identify its component names and start their registered loaders while the remaining data continues streaming. Load implementations revealed by later Python-component results when those results arrive; first-render loading remains the fallback for conditional or otherwise undiscovered content. Speculative loading must not render components or execute route actions. Setup imports remain eager, and lazy action loading needs a separate invocation contract.

...
A default-export implementation becomes a direct `js_source`. A proven named export gets a small default-export adapter under `js/dara-adapters/`. Python class metadata is replaced in place, runtime naming overrides remain, and explicit registrations remain with only the obsolete `local=True` removed. Existing literal `js_source` declarations also allow that registration cleanup. npm declarations are converted only when a locally available package manifest and barrel prove a corresponding default-export subpath.

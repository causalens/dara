...
config.add_page('Authentication Demo', content=page_content)
```
...
config.add_page('Weather Demo', content=page_content)
```
...
config.add_page('Role Demo', content=page_content)
```
...
#### State Tracking with `StateVariable`

`DerivedVariable`s provide three built-in state tracking properties that return `StateVariable` instances to help you monitor the execution state of your derived calculations:
...
```python
from dara.core import Variable, DerivedVariable
from dara.components import Stack, Text, Button, Input, If, Spinner
...
:::note
`StateVariable` instances are read-only and automatically managed by their parent `DerivedVariable`.
They also cannot be used as inputs to other `DerivedVariable`s to prevent complex dependency cycles.
:::

...
<details><summary>What can you store in a <code>Variable</code>?</summary>
...
The components mentioned above offer a way to directly update Variables. Another way to update Variables is through actions. Actions can be found in the core framework along with `Variable` and they work by being passed to a component's callback. A callback is simply a function that is passed as an argument to another function.

#### `UpdateVariable`

The `dara.core.interactivity.actions.UpdateVariable` action can be passed to any component prop accepting an Action and trigger the update of a `dara.core.interactivity.plain_variable.Variable`, `dara.core.interactivity.data_variable.DataVariable` or `dara.core.interactivity.url_variable.UrlVariable`.
...
The `dara.components.common.button.Button` component has a parameter `onclick` which is the component's callback. You can pass it an action, which in this case is an `UpdateVariable` instance.
...
Other components like `Select`s have corresponding callbacks. The callback for a `dara.components.common.select.Select` component is the `onchange` argument.

In summary, to trigger an update based on user interaction simply pass an action to your component's callback. `UpdateVariable` is just one of the actions to inspire you on what you can do with your app. To see more actions checkout the [Actions](./actions.md).
...
<details><summary><h3 style={{margin: '0px 0px 0px 0px'}}> Tasks:<code>DerivedVariables</code> with Heavy CPU Calculations</h3></summary>
...
You can achieve this using the `dara.core.interactivity.plain_variable.create_from_derived` static method on the `Variable` class. It creates a new `Variable` instance that is initially bound to the `DerivedVariable` passed as an argument. The resulting `Variable` will be mutable, like any other `Variable`. At first, it will mirror the value of the `DerivedVariable`. Once it's explicitly modified, e.g. by a component or an `UpdateVariable` action, it will stop mirroring the `DerivedVariable` value.
...
<details><summary>What is a python decorator?</summary>

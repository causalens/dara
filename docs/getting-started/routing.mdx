...
### Route Metadata and IDs

You are free to add any metadata to your routes. This can be used to store arbitrary data that can later be used within reusable layouts or `on_load` functions.
...
config.router.add_page(
    ...
    id="admin-panel",
    name="Administration",
    metadata={"requires_auth": True, "role": "admin"}
)
```

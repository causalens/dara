...
@action
async def my_resolver_function(ctx: action.Ctx, my_data_var):
    # my_data_var here takes the value of my_first_data_variable
    ...
    ctx.update(variable=target, value=my_data_var)

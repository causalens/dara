...
def plot_data(data: pandas.DataFrame)
    ...
    p.line(x=data['X Data'], y=data['Y Data'])
    return Bokeh(p)
...
The data extension provides extra functionalities for interacting with data, built on top of the core `DataVariable` concepts.
The extension also contains a `DataFactory` class which is a factory of variables, actions and methods to interact with locally stored data.

...
from dara.core.data_utils import DataFactory, UploadDropzone
...
def create_df(ctx: SideEffect.Ctx):
    ...
    uid = str(uuid4())
    ds_factory.write_dataset(df, f'random_{uid}', cache=sel_cache)
...
def add_data_section():
    return Stack(
        Stack(
            ...
            Spacer(),
            Button('Create random dataset for given cache', onclick=SideEffect(create_df, extras=[selected_cache])),
        ),
        Spacer(),
        Stack(

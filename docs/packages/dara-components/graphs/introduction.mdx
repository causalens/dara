...
**Themed Colors** - These are specified with a `theme.` prefix, followed by the color key from our default theme palette.
For example, `theme.primary`, `theme.secondary`, or any other predefined theme color.
...
- `dash_array: string` - Optional dashArray SVG path property - line will be dashed if specified

...
For [TimeSeriesCausalGraph](https://causalgraph.causalens.com/docs/generated/cai-causal-graph/docs/time_series_causal_graph) we have an added in support where for layouts that support tiers, they will use `time_lag` and `variable_name` to place these nodes within the same tier.
This is currently supported by `FcoseLayout`, `SpringLayout` and `MarketingLayout`.

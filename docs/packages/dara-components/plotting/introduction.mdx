...
### Add your first `dara.components.plotting.bokeh.bokeh.Bokeh` Component

Now that you are setup we can go ahead into adding our first Bokeh plot.
...
:::tip
A useful prop to set to your `Bokeh` figure is `sizing_mode`, this allows for the `Bokeh` plot to adapt its size based on the space available to it.
...
### Add your first `dara.components.plotting.plotly.plotly.Plotly` Component

For Plotly all you need to do is pass a [figure](https://plotly.com/python/figure-structure/) to the component. This component can be used for Plotly Express as well.
...
```python
from dara.components.plotting import set_default_bokeh_theme, set_plotly_default_theme

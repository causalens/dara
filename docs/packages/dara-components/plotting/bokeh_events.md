...
from dara.components.plotting.utils import figure_events
from dara.core import Variable, UpdateVariable, py_component, NavigateTo
from dara.components import Stack, Text, If, Button, Bokeh
...
```python
    ...
    # the `new` parameter of the UpdateVariable resolver will hold the value returned by the callback
    # in this case the value will hold the wheat_class of whatever plot point was selected
    return Bokeh(p, events=[
        ('CLICK', [UpdateVariable(lambda ctx: ctx.inputs.new, selected_variety_var)])
    ])
```
...
from dara.core import ConfigurationBuilder, Variable, UpdateVariable, py_component, NavigateTo
from dara.components import Stack, Text, If, Button, Card, Bokeh
from dara.components.plotting.utils import figure_events
    ...
    if selected_variety is None:
        ...
        return Stack(
            ...
            Button(
                ...
                onclick=NavigateTo(url=f'https://en.wikipedia.org/wiki/Triticum_{selected_variety}', new_tab=True),
            ),
            height='10%',
            align='center'
        )
...
from dara.core import Variable, UpdateVariable, py_component, ConfigurationBuilder, get_icon
from dara.components import Stack, Text, Card, Bokeh
from dara.components.plotting.utils import figure_events
...
config = ConfigurationBuilder()
config.add_page(name='Lasso Example', content=lasso_page(), icon=get_icon('hat-cowboy-side'))
```

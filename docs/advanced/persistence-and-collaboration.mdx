...
By default `Variable` state is purely client-side - by default its source of truth is the browser memory.
It also means that refreshing the browser page resets the `Variable` state, and that each user has their own 'copy'
of the data. In order to make the aforementioned default behaviour more customizable to suit different needs, `Variable` accepts
a `store` property. It can be used to customize the source of truth for a given `Variable` and enables
...
The behaviour of the store can be customized by passing arguments to the `BackendStore`.
...
When using `user` scope, each user has their own copy of the date. A change made to an attached `Variable` in one client will

...
To easily support `Variable` type arguments (including `UrlVariable` and `DerivedVariable`) there is a helper hook called
...
a TypeScript `ActionHook` helper type is provided:
...
export interface ActionHook<
  ActionReturnType,
  ActionInstanceType extends ActionInstance = ActionInstance
> {
  (
    action: ActionInstanceType,
    actionContext: ActionContext<ActionReturnType>
...
constructs and functions. The action function can be thought of as a `React` hook, that returns a callback to execute the action.

```tsx = title=js/custom-action.tsx
import { useCallback } from "react";
import { ActionHook, ActionInstance } from "@darajs/core";

interface CustomActionInstance extends ActionInstance {
  country: string;
}

const CustomAction: ActionHook<string, CustomActionInstance> = (
  action,
  { fetchAction }
) => {
  return useCallback(async (name: any) => {
    const fullName = `${name} from ${action.country}`;
    return fetchAction(action.uid, { inputs: { value: fullName } });
  }, []);
};
...
In this example, you define an action handler for an instance that can be configured with a `country` prop. When the action is called
with a value coming from a component, that value is combined with the `country` prop and calls the backend with the combined value.
...
To register the action on the backend side you need to write a subclass of `ActionInstance` and create it with your application config.
...
class CustomAction(ActionInstance):
    ...
    def __init__(self, function: Callable[[ComponentActionContext], Any]):
        """
        :param function: the function to be called
        """
        uid = str(uuid.uuid4())

        # You need to register the handler for the function
        self.register_resolver(uid, function)

        super().__init__(uid=uid, function=function)


config.add_action(CustomAction, local=True)
```

Note that if you want to execute backend code when the action is called with `fetchAction` from the frontend, the backend
`ActionInstance` needs to take in a `Callable[[ActionContextType], Any]` parameter and register it by calling `self.register_resolver`.

`ActionContext` requires two fields `extras` and `inputs`. You can define your own custom context depending on your usecase.
If it only takes one value as input, then you can simply use `ComponentActionContext`, which intends to take the component value
to the resolving function, for example the selected value on a select component.

:::tip
The action can be purely front-end and does not need to call the backend at all. You can then skip the `__init__` part
of the backend definition.
:::

The action can then be used like any other built-in action.

```python
Button('Test button', onclick=CustomAction(country='United Kingdom', function=lambda x: f'Greetings, {x}!'))
```

...
You can follow the [User Guide](./getting-started/whats-dara) for more details on how to install the packages and create your first app.

Your application code structure should have the following pattern:
...
The `pyproject.toml` file has the information about the name of the application and for the above example should be:
...
At this point the code behind your app was generated by the CLI and it looks like the following:
...
Your `main.py` file is where you want to set up your configuration with the `dara.core.configuration.ConfigurationBuilder`.

In this example, the `ConfigurationBuilder` is adding your pages to your app.
...
It is good to have your global state in one place and to keep it out of `main.py` for organization and readability. For this reason, you will define your dataset in `definitions.py`.
...
from dara.components.palettes import CategoricalLight3
...
So far you've made a dynamic page for some simple exploratory data analysis. You may also want to do some simple modelling with the iris dataset.
...
```python title=pages/classification.py
import pandas
...
from dara.components import Bokeh, Card
from dara.components.palettes import SequentialDark8
...
You can take advantage of a `DerivedVariable` to calculate your predictions based the `Variable`, `max_depth_var`. You can simply move the logic that calculates your predictions that you already have into a function called `calculate_predictions` which can act as the function needed by the `DerivedVariable`.
